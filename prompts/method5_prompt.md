...
- the **source schema** and the **source query**: the query's origin. The source
  schema is a *different*, independently designed database whose tables are
  defined over the target schema, and the source query is the query as it was
  originally written there. The target query is a mechanical migration of that
  source query.
...
The target query was produced mechanically, by inlining a table definition per
source table, so it is correct but usually far from minimal: it may materialize
whole tables as CTEs, join tables it never reads a column from, aggregate or sort
rows that are discarded later, and repeat the same scan several times. Your job is
to find the query a competent engineer would have written by hand for that same
result.
...
applied late. Use the source schema and source query to see *what the query is
actually asking*: the inlined structure of the target query mirrors the source
tables rather than the question, so the source query shows which columns and
relationships the result really depends on, and therefore which of that structure
is incidental. Target those parts first (e.g. inline or drop a CTE, push
...
- Preserve the **target** query's output: same columns, in the same order, with
  the same column names/aliases, the same row multiplicity, and the same
  ORDER BY / LIMIT behaviour. Where the source query and the target query seem to
  disagree, the target query is the specification.
