...
from db_tools import get_table_schema, get_table_samples, get_query_plan
...
def extract_tables(query: str) -> list[str]:
  ...
  try:
    ...
    return list({node.name for node in parsed.walk() if isinstance(node, exp.Table) and node.name})
  except Exception:
    return []
...
def main() -> None:
  ...
  if not os.path.exists(raw_path):
    raise FileNotFoundError(
      f"Could not find {raw_path!r}. Run generate_raw_queries.py first."
    )
  ...
  with psycopg.connect(DB_URL, autocommit=True) as conn:
    for q in raw_data["queries"]:
      ...
      try:
        query_plan = get_query_plan(conn, query)
